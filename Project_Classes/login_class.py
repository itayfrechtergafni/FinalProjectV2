...
class TalkHobiLogin(ctk.CTkFrame):
    ...
    def login_button_command(self):
        ...
        self.query_sock.sendto(
            ...
            self.server_addr)
        query_answer = self.query_sock.recvfrom(CHUNK_SIZE)[0].split(SEP)
        print(query_answer)
        if query_answer[0] == sql_query_flags['username_login']:
            if query_answer[1] == b'True':
                ...
                print("Logging in...")
                self.query_sock.sendto(sql_query_flags['user_ID_by_username']+SEP+field1.encode(), self.server_addr)
                query_answer, _ = self.query_sock.recvfrom(CHUNK_SIZE)
